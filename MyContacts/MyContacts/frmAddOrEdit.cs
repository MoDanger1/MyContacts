...
namespace MyContacts
{
    public partial class frmAddOrEdit : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            if (ValidateInputs())
            {
                bool isSuccess = repository.Insert(txtName.Text, txtFamily.Text, txtMobile.Text, (int)txtAge.Value, txtEmail.Text, txtAddress.Text);
                if (isSuccess == true)
                {
                    MessageBox.Show("عملیات با موفقیت انجام شد", "ثبت شد", MessageBoxButtons.OK, MessageBoxIcon.Information);
